...
async function scrapeChapter(chapterNum) {
    ...
    const url = chapterNum === 1 
        ? "https://augustino.net/tin-mung-theo-thanh-mac-co" 
        : `https://augustino.net/tin-mung-theo-thanh-mac-co-chuong-${chapterNum}`;
...
async function main() {
    console.log("Starting Mark scraper using Node.js...");
    const allVerses = [];
    ...
    fs.writeFileSync('scripts/scraped_mark.json', JSON.stringify(allVerses, null, 2));
    
    // Generate SQL file
    let sqlContent = `-- Dữ liệu Tin Mừng Mác-cô (${allVerses.length} câu)\n`;
    ...
    fs.writeFileSync('database/mark_insert.sql', sqlContent);
    console.log("Generated database/mark_insert.sql successfully!");
}

...
const BOOK_ID = 'dc'; // Ký hiệu sách Diễm Ca (Dc)
const TOTAL_CHAPTERS = 8;
const URL_SLUG = 'sach-diem-ca';
...
function parseChapterContent($, contentDiv) {
    ...
    contentDiv.children().each((_, el) => {
        ...
        if (tag === 'p') {
            ...
            // 2. Tiêu đề Tiểu mục / Người xướng [SECTION] (ví dụ: NÀNG, CHÀNG, BẠN HỮU)
...
async function scrapeAllSongOfSongs() {
    console.log(`🚀 Bắt đầu cào 8 chương Sách Diễm Ca (${BOOK_ID})...`);
    const chaptersData = [];
    const sqlStatements = [];
    ...
    fs.writeFileSync('scripts/song_of_songs_all_chapters.json', JSON.stringify(chaptersData, null, 2), 'utf-8');
    fs.writeFileSync('database/song_of_songs_all_chapters.sql', sqlStatements.join('\n\n'), 'utf-8');

    console.log(`🎉 HOÀN THÀNH! Đã cào chuẩn nguyên văn 100% 8 chương Sách Diễm Ca.`);
    console.log(`📄 Tệp JSON: scripts/song_of_songs_all_chapters.json`);
    console.log(`📄 Tệp SQL: database/song_of_songs_all_chapters.sql`);
}

scrapeAllSongOfSongs();

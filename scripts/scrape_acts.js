...
async function scrapeChapter(chapterNum) {
    ...
    const url = chapterNum === 1 
        ? "https://augustino.net/sach-cong-vu-tong-do" 
        : `https://augustino.net/sach-cong-vu-tong-do-chuong-${chapterNum}`;
...
async function main() {
    console.log("Starting Acts of the Apostles scraper using Node.js...");
    const allVerses = [];
    ...
    fs.writeFileSync('scripts/scraped_acts.json', JSON.stringify(allVerses, null, 2));
    
    let sqlContent = `-- Dữ liệu Sách Công Vụ Tông Đồ (${allVerses.length} câu)\n`;
    ...
    fs.writeFileSync('database/acts_insert.sql', sqlContent);
    console.log("Generated database/acts_insert.sql successfully!");
}

...
3. Điền các giá trị vào tệp .env (tham khảo file .env.example):
   R2_ACCOUNT_ID=...
   R2_ACCESS_KEY_ID=...
...
function getCacheControl(filePath) {
  const ext = path.extname(filePath).toLowerCase();
  // Với file MP3/WAV tĩnh và phụ đề: Cache 1 năm trên CDN và Browser
  if (['.mp3', '.wav', '.m4a', '.aac', '.ogg', '.flac', '.srt', '.vtt'].includes(ext)) {
    return 'public, max-age=31536000, immutable';
  }
  // Với file manifest JSON: Cache 5 phút, sau đó revalidate
  if (ext === '.json') {
...
// ── UPLOAD 1 FILE VỚI CƠ CHẾ RETRY ───────────────────────────────────
async function uploadFileWithRetry(s3Client, fileObj, bucketName, maxRetries = 3) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Nghỉ tăng dần trước khi thử lại
      await new Promise((resolve) => setTimeout(resolve, attempt * 1000));
    }
  }
}

// ── CHẠY NHIỀU LUỒNG UPLOAD (CONCURRENCY QUEUE) ──────────────────────
...
async function main() {
  ...
  // 3. Lấy remote objects để so khớp
  ...
  // Nếu là dry-run: chỉ hiển thị danh sách
  if (IS_DRY_RUN) {

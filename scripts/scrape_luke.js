...
async function scrapeChapter(chapterNum) {
    ...
    const url = chapterNum === 1 
        ? "https://augustino.net/tin-mung-theo-thanh-lu-ca" 
        : `https://augustino.net/tin-mung-theo-thanh-lu-ca-chuong-${chapterNum}`;
...
async function main() {
    console.log("Starting Luke scraper using Node.js...");
    const allVerses = [];
    ...
    fs.writeFileSync('scripts/scraped_luke.json', JSON.stringify(allVerses, null, 2));
    
    let sqlContent = `-- Dữ liệu Tin Mừng Lu-ca (${allVerses.length} câu)\n`;
    ...
    fs.writeFileSync('database/luke_insert.sql', sqlContent);
    console.log("Generated database/luke_insert.sql successfully!");
}

...
async function main() {
    console.log("===============================================================");
    console.log("🚀 BẮT ĐẦU ĐẨY DỮ LIỆU KINH THÁNH LÊN SUPABASE");
    console.log("===============================================================");

    // 1. Đảm bảo bản dịch Các Giờ Kinh Phụng Vụ có id = 1
    console.log("\n📖 1. Kiểm tra bản dịch 'Các Giờ Kinh Phụng Vụ' (id=1)...");
    ...
    if (transErr) {
        console.error("  ⚠️ Lỗi khi nạp translation:", transErr.message);
    } else {
        console.log("  ✅ Translation sẵn sàng.");
    }

    // 2. Nạp danh sách 73 sách
    console.log("\n📚 2. Nạp metadata 73 cuốn sách vào bảng `books`...");
    ...
    if (booksErr) {
        console.error("  ❌ Lỗi khi nạp books:", booksErr.message);
    } else {
        console.log(`  ✅ Đã nạp thành công ${booksRows.length} sách vào bảng books.`);
    }

    // 3. Đẩy toàn bộ 1.328 chương vào bảng `chapters`
    console.log("\n📦 3. Đẩy 1.328 chương vào bảng `chapters`...");
    let totalPushed = 0;
    let failedChapters = 0;

    for (let i = 0; i < BOOKS_METADATA.length; i++) {
        const book = BOOKS_METADATA[i];
        if (!fs.existsSync(book.jsonFile)) {
            console.error(`  ❌ Không tìm thấy ${book.jsonFile}`);
            continue;
        }

        const raw = fs.readFileSync(book.jsonFile, 'utf-8');
        ...
        const { error: chapErr } = await supabase
            .from('chapters')
            .upsert(rows, { onConflict: 'translation_id,book_id,chapter' });

        if (chapErr) {
            console.error(`  ❌ [${i + 1}/73] Lỗi sách ${book.name} (${book.id}):`, chapErr.message);
            failedChapters += rows.length;
        } else {
            totalPushed += rows.length;
            console.log(`  ✅ [${i + 1}/73] ${book.name} (${book.id}): đã nạp ${rows.length}/${book.total_chapters} chương`);
        }
    }

    console.log("\n===============================================================");
    console.log(`🎉 HOÀN THÀNH ĐẨY DỮ LIỆU LÊN DATABASE SUPABASE!`);
    console.log(`- Tổng số chương đã nạp thành công: ${totalPushed} / 1.328`);
    console.log(`- Số chương thất bại: ${failedChapters}`);
    console.log("===============================================================");
}

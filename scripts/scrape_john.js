...
async function scrapeChapter(chapterNum) {
    ...
    const url = chapterNum === 1 
        ? "https://augustino.net/tin-mung-theo-thanh-gio-an" 
        : `https://augustino.net/tin-mung-theo-thanh-gio-an-chuong-${chapterNum}`;
...
async function main() {
    console.log("Starting John scraper using Node.js...");
    const allVerses = [];
    ...
    fs.writeFileSync('scripts/scraped_john.json', JSON.stringify(allVerses, null, 2));
    
    let sqlContent = `-- Dữ liệu Tin Mừng Gio-an (${allVerses.length} câu)\n`;
    ...
    fs.writeFileSync('database/john_insert.sql', sqlContent);
    console.log("Generated database/john_insert.sql successfully!");
}
